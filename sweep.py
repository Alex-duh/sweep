...
def determine_tier(reason: str) -> str:
    """
    Decide from the reason string whether the decision was made by a rule
    (fast, deterministic) or by Groq (LLM call).
    Used for the stats summary and log entries.
    """
    return "rule" if reason.startswith("rule:") or reason.startswith("no college") else "groq"
...
def main():
    ...
    for msg_ref in tqdm(msg_ids, desc="Scanning", unit="email"):
        ...
        # Collect the log entry — no body content logged.
        email_log_entries.append({
            "sender":      email["sender"],
            "subject":     email["subject"],
            "label":       label,
    ...
    groq_n  = sum(1 for e in decided if e["tier"] == "groq")
    groq_confs = [e["confidence"] for e in decided if e["tier"] == "groq"]
    avg_conf   = sum(groq_confs) / len(groq_confs) if groq_confs else 0.0
    rule_pct   = rule_n / len(decided) * 100 if decided else 0
    groq_pct   = groq_n / len(decided) * 100 if decided else 0
    ...
    stats.add_row("Groq decisions (of in-scope)",     f"{groq_n} of {n_decided}  ({groq_pct:.0f}%)")
    stats.add_row("Avg Groq confidence",      f"{avg_conf:.0%}" if groq_confs else "n/a")
    console.print(stats)
