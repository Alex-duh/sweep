...
Runs three tiers in order, stopping as soon as a tier decides:
  Tier 1: is_definite_keep()       — strong personal signals  → keep, no LLM
  Tier 2: is_definite_recruitment() — strong spam signals     → archive, no LLM
  Tier 3: ask_groq()               — truly ambiguous          → ask LLM

The top-level classify() function calls all three and returns a unified dict.
"""
...
# cases fall through to Groq instead.
RECRUITMENT_PHRASES = [
...
# ---------------------------------------------------------------------------
# Tier 3 — ambiguous → Groq LLM
# ---------------------------------------------------------------------------

def ask_groq(email: dict, my_name: str = "Alex") -> dict:
    """
    Sends the email to Groq (Llama 3.1 8B) for classification.
    Only called when both rule tiers returned False (truly ambiguous).

    Returns: {"label": "keep" | "recruitment", "confidence": 0.0–1.0}

    Why temperature=0?
    We want consistent, deterministic answers — not creative ones.

    Why max_tokens=50?
    We only need a tiny JSON blob back. Fewer tokens = faster + cheaper.
    """
    client = Groq(api_key=os.environ["GROQ_API_KEY"])

    # Send only a short preview of the body to save tokens.
    # 400 chars is enough for the LLM to detect tone and signals.
    preview = email.get("body", "")[:400]

    prompt = f"""You are helping {my_name} clean their inbox of college recruitment spam.

Classify this email as "recruitment" or "keep":

"recruitment" means: a mass outreach email from a college/university trying to get {my_name} to apply, visit campus, or explore the school. Generic greeting, no specific application context.
...
"keep" means: anything that is NOT college recruitment — including:
  ...
  - Newsletters or digests not about recruiting {my_name} to apply
...
{{"label": "recruitment" or "keep", "confidence": 0.0 to 1.0}}"""
    ...
    response = client.chat.completions.create(
        model="llama-3.1-8b-instant",  # free on Groq, fast
    ...
    # If the model returns garbage instead of JSON, crash loudly here
    # rather than silently mislabeling an email downstream.
    try:
        return json.loads(raw)
    except json.JSONDecodeError:
        # Fallback: if we can't parse, err on the side of keeping the email.
        # tqdm.write keeps the progress bar intact; plain print would clobber it.
        from tqdm import tqdm as _tqdm
        _tqdm.write(f"  [warn] Groq returned non-JSON: {raw!r} — defaulting to keep")
...
def classify(email: dict, my_name: str = "Alex") -> dict:
    ...
    Classifies one email. Returns:
      {
        "label":      "keep" | "recruitment" | "skip_review",
        "reason":     human-readable explanation,
        "confidence": float 0.0–1.0
      }

    "skip_review" means the LLM thought it was recruitment but wasn't
    confident enough (< 0.8). sweep.py logs these separately for manual review.
    ...
    if not is_college_related(email):
        if ".edu" in sender:
            ...
            # Rules can't tell them apart here — let Groq decide.
            # The Groq prompt explicitly names commercial .edu platforms as non-recruitment.
            result     = ask_groq(email, my_name)
            label      = result.get("label", "keep")
            confidence = float(result.get("confidence", 0.0))
            if label == "recruitment" and confidence < 0.8:
                return {
                    "label": "skip_review",
                    "reason": f"LLM (.edu, no keywords): recruitment but only {confidence:.0%} confident",
                    "confidence": confidence,
                }
            return {
                "label": label,
                "reason": f"LLM (.edu, no keywords): {confidence:.0%} confident",
                "confidence": confidence,
            }
    ...
    # Passed scope gate, no rule fired — ask the LLM.
    result     = ask_groq(email, my_name)
    label      = result.get("label", "keep")
    confidence = float(result.get("confidence", 0.0))
    ...
    if label == "recruitment" and confidence < 0.8:
        return {
            "label": "skip_review",
            "reason": f"LLM: recruitment but only {confidence:.0%} confident",
            "confidence": confidence,
        }
    return {
        "label": label,
        "reason": f"LLM: {confidence:.0%} confident",
        "confidence": confidence,
    }
...
if __name__ == "__main__":
    samples = [
        ...
        },
        {
            ...
            # and no "campus" in body → apply_now_counts=False → falls to Groq, not archived.
            "name": "Academia.edu upsell (KEEP or Groq — never recruitment)",
        ...
        {
            "name": "College email with unsubscribe but no CTA (should be KEEP or Groq)",
            "sender": "news@mit.edu",
    ...
    print("--- Scope gate check (no Groq calls) ---\n")
    scope_cases = [
    ...
    print("\n--- Full classification (last sample calls Groq) ---\n")
    for s in samples:
